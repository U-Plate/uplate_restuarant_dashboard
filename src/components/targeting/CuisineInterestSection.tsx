...
import { CUISINE_INTEREST_SUGGESTIONS } from '../../data/constants';
import type { CuisineInterestRule } from '../../types';

interface Props {
  value: CuisineInterestRule[];
  onChange: (next: CuisineInterestRule[]) => void;
  isOpen: boolean;
  onToggle: () => void;
}

export function CuisineInterestSection({ value, onChange, isOpen, onToggle }: Props) {
  ...
  const suggestions = useMemo(() => {
    ...
    if (!q) return [];
    return CUISINE_INTEREST_SUGGESTIONS.filter(
  ...
  return (
    <TargetingSection
      ...
      eyebrow="Cuisine interests"
      hint="Free-text cuisines or culinary styles you serve."
      summary={summary}
      isOpen={isOpen}
      onToggle={onToggle}
      isEmpty={value.length === 0}
      emptyCta="Add cuisines"
    >
      <div className="uplate-targeting__search">
        ...
        <input
          ...
          placeholder="Search cuisines, or type your own"
...
function summarize(value: CuisineInterestRule[]): string {
